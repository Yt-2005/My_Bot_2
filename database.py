...
Provides:
  • get_db_url()       — resolve DB URL from env vars
  • get_connection()   — raw psycopg connection (dict_row)
  • init_db()          — create / migrate all tables (called on startup)
  • User CRUD
  • Expense CRUD
  • Notes CRUD
...
def get_db_url() -> str:
    """Read DB URL from any of the supported env var names."""
    return (
        os.environ.get("SUPABASE_DB_URL") or
        os.environ.get("DATABASE_URL") or
        os.environ.get("POSTGRES_URL") or
...
def get_connection():
    """Return a fresh psycopg connection with dict_row factory."""
    url = get_db_url()
    if not url:
        raise RuntimeError(
            "❌ No DB URL found! Set SUPABASE_DB_URL (or DATABASE_URL) "
            "in your Render environment variables."
        )
    return psycopg.connect(url, row_factory=dict_row)


# Alias used by some bot code
def db():
...
def init_db():
    """
    Create every table the bot and dashboard depend on.
    ...
    try:
        ...
        conn.commit()
        logger.info("✅ init_db: all tables ready")
...
def get_or_create_user(user_id: int, username: str = None, language: str = "km") -> dict:
...
def update_user(user_id: int, **fields) -> bool:
    """Update any subset of user fields. Pass field=value kwargs."""
    if not fields:
        return False
    allowed = {"username", "language", "budget", "daily_reminder", "reminder_time", "pin"}
...
def delete_user(user_id: int) -> bool:
    """Delete user and all their data (cascades to expenses/notes)."""
    conn = get_connection()
    try:
        cur = conn.cursor()
        cur.execute("DELETE FROM chat_memory WHERE user_id = %s", (user_id,))
        cur.execute("DELETE FROM error_logs WHERE user_id = %s", (user_id,))
...
def add_expense(user_id: int, category: str, amount: float,
                ...
                expense_date: date = None) -> Optional[dict]:
    conn = get_connection()
    try:
        cur = conn.cursor()
        cur.execute(
            ...
            RETURNING *
            """,
            (user_id, category, amount, note, tag, expense_date or date.today()),
        )
        conn.commit()
        return cur.fetchone()
    except Exception as e:
...
def get_expenses(user_id: int, limit: int = 50, offset: int = 0) -> list:
    conn = get_connection()
    try:
        cur = conn.cursor()
        cur.execute(
...
def get_monthly_total(user_id: int, year: int, month: int) -> float:
    conn = get_connection()
    try:
        cur = conn.cursor()
        cur.execute(
...
def delete_last_expense(user_id: int) -> Optional[dict]:
    """Delete the most recent expense for a user and return it."""
    conn = get_connection()
    try:
        cur = conn.cursor()
        cur.execute(
        ...
        conn.rollback()
        logger.error(f"delete_last_expense error: {e}")
        return None
    finally:
        conn.close()
...
def add_note(user_id: int, content: str) -> Optional[dict]:
    conn = get_connection()
    try:
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO notes (user_id, content) VALUES (%s, %s) RETURNING *",
            (user_id, content),
        )
        conn.commit()
        return cur.fetchone()
...
def get_notes(user_id: int, limit: int = 20) -> list:
    conn = get_connection()
    try:
        cur = conn.cursor()
        cur.execute(
            "SELECT * FROM notes WHERE user_id = %s ORDER BY created_at DESC LIMIT %s",
            (user_id, limit),
        )
        return cur.fetchall()
    finally:
...
def get_admin_role(user_id: int) -> Optional[str]:
    """Return the role string ('superadmin'/'admin'/'moderator') or None."""
    conn = get_connection()
    try:
        cur = conn.cursor()
        cur.execute("SELECT role FROM bot_admins WHERE user_id = %s", (user_id,))
        row = cur.fetchone()
...
def set_admin(user_id: int, role: str = "admin",
              username: str = None, note: str = None,
    ...
    try:
        ...
        cur.execute(
            """
            INSERT INTO bot_admins (user_id, username, role, note, added_by)
            VALUES (%s, %s, %s, %s, %s)
            ON CONFLICT (user_id) DO UPDATE
            ...
            """,
            (user_id, username, role, note, added_by),
        )
        conn.commit()
        return True
    except Exception as e:
...
def get_all_admins() -> list:
    ...
    try:
        cur = conn.cursor()
        cur.execute("SELECT * FROM bot_admins ORDER BY added_at")
        return cur.fetchall()
    finally:
        conn.close()
...
def log_error(user_id: int, error: str, context: str = None):
    """Insert a row into error_logs. Never raises — safe to call anywhere."""
    try:
        conn = get_connection()
        try:
            cur = conn.cursor()
            cur.execute(
                ...
                (user_id, str(error)[:2000], context),
            )
            conn.commit()
        finally:
            conn.close()
    except Exception as e:
        logger.warning(f"log_error failed (non-fatal): {e}")
...
def save_chat_memory(user_id: int, role: str, content: str):
    ...
    try:
        ...
        conn.rollback()
        logger.error(f"save_chat_memory error: {e}")
    finally:
        conn.close()


def get_chat_memory(user_id: int, limit: int = 20) -> list:
    conn = get_connection()
    try:
        cur = conn.cursor()
        cur.execute(
...
def clear_chat_memory(user_id: int):
    conn = get_connection()
    try:
        cur = conn.cursor()
        cur.execute("DELETE FROM chat_memory WHERE user_id = %s", (user_id,))
        ...
        conn.rollback()
        logger.error(f"clear_chat_memory error: {e}")
    finally:
        conn.close()
...
def get_reminder_users() -> list:
    """Return all users who have daily_reminder enabled."""
    conn = get_connection()
    try:
        cur = conn.cursor()
        cur.execute(
            "SELECT user_id, username, reminder_time, language FROM users WHERE daily_reminder = TRUE"
        )
        return cur.fetchall()
    finally:
        conn.close()
...
def set_reminder(user_id: int, enabled: bool, reminder_time: str = "20:00") -> bool:
    return update_user(user_id, daily_reminder=enabled, reminder_time=reminder_time)


# ─────────────────────────────────────────────────────────────────
# DB STATS (for /dbstats command)
# ─────────────────────────────────────────────────────────────────

def get_db_stats() -> dict:
    """Return row counts for every table."""
    tables = ["users", "expenses", "notes", "banned_users",
              "bot_admins", "error_logs", "chat_memory"]
    conn = get_connection()
    stats = {}
    try:
        cur = conn.cursor()
