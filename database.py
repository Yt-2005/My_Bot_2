...
Functions exported:
  init_db()
  get_conn()
  ensure_user(user_id, username, language)
  ...
  unban_user(user_id)
  is_banned(user_id)
  send_reminders_db(hour_str)
  log_error(user_id, error, context)
"""
...
def get_conn():
    """Return a fresh psycopg connection (dict_row). Always close after use."""
    url = _get_db_url()
    if not url:
        raise RuntimeError(
            "❌ No DB URL found. Set SUPABASE_DB_URL (or DATABASE_URL) in your environment."
        )
    return psycopg.connect(url, row_factory=dict_row)
...
def init_db():
    """Create all required tables if they don't exist. Called once in bot.py main()."""
    conn = get_conn()
    try:
        cur = conn.cursor()
        ...
        # Chat memory (AI conversation history)
        cur.execute("""
...
def ensure_user(user_id: int, username: str = "", language: str = "km") -> dict:
    """
    Insert the user if not present, or update username. Returns the user row.
    Called at every /start and on every handler that needs a user record.
    """
    conn = get_conn()
    try:
        conn.execute("""
...
def get_user_info(user_id: int) -> dict | None:
    """
    Return a dict with user profile + aggregated spend info.
    ...
    try:
        ...
        result = dict(user)
        result["total_spent"]    = float(row["total_spent"]) if row else 0.0
        result["expense_count"]  = int(row["expense_count"]) if row else 0
        return result
    except Exception as e:
...
def save_chat_message(user_id: int, role: str, content: str):
    """Append one message to chat_memory."""
    conn = get_conn()
    try:
        conn.execute(
...
def clear_chat_history(user_id: int):
    """Delete all chat memory for a user (/clearchat command)."""
    conn = get_conn()
    try:
        conn.execute("DELETE FROM chat_memory WHERE user_id = %s", (user_id,))
        conn.commit()
    except Exception as e:
        conn.rollback()
...
def send_reminders_db(hour_str: str) -> list[int]:
    """
    Return list of user_ids who have daily_reminder ON and
    whose reminder_time starts with the given hour string (e.g. '08').
    Called by the PTB job queue every hour.
    """
    conn = get_conn()
    try:
        rows = conn.execute("""
            ...
            WHERE daily_reminder = TRUE
              AND reminder_time LIKE %s
        """, (f"{hour_str}:%",)).fetchall()
        return [r["user_id"] for r in rows]
    except Exception as e:
        logger.error(f"send_reminders_db error: {e}")
        return []
    finally:
        conn.close()
...
def get_bot_stats() -> dict:
    """
    Return a summary dict used by /botstats admin command.
    """
    conn = get_conn()
    try:
        def _one(sql, params=()):
            ...
            if row is None:
                return 0
            # dict_row: grab first value
            return list(row.values())[0]

        stats = {
            "total_users":    _one("SELECT COUNT(*) FROM users"),
            "new_today":      _one("SELECT COUNT(*) FROM users WHERE created_at::date = CURRENT_DATE"),
            "total_spent":    float(_one("SELECT COALESCE(SUM(amount), 0) FROM expenses")),
            "total_expenses": _one("SELECT COUNT(*) FROM expenses"),
            "total_notes":    _one("SELECT COUNT(*) FROM notes"),
            "errors":         _one("SELECT COUNT(*) FROM error_logs"),
            "banned":         _one("SELECT COUNT(*) FROM banned_users"),
            "active_30d":     _one("""
        ...
        return {
            "total_users": 0, "new_today": 0, "total_spent": 0.0,
            "total_expenses": 0, "total_notes": 0, "errors": 0,
            "banned": 0, "active_30d": 0,
        }
    finally:
        conn.close()
...
def log_error(user_id: int | None, error: str, context: str = ""):
    """Save an error to error_logs. Safe to call from exception handlers."""
    conn = get_conn()
    try:
        conn.execute(
            "INSERT INTO error_logs (user_id, error, context) VALUES (%s, %s, %s)",
            (user_id, str(error)[:2000], context[:200])
        )
        conn.commit()
    except Exception as e:
        logger.warning(f"log_error failed: {e}")
    finally:
        conn.close()